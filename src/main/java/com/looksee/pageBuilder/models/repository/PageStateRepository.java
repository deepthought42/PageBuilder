...
public interface PageStateRepository extends Neo4jRepository<PageState, Long> {
	...
	@Query("MATCH (d:Domain) WITH d WHERE id(d)=$audit_record_id MATCH (d)-[:HAS]->(page:PageState) WHERE page.key=$page_key RETURN page")
	public PageState findPageWithKey(@Param("domain_id") long domain_id, @Param("page_key") String key);
